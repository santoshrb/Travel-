module EnquiriesHelper
	def all_enquiry
		if current_user.role.name == "Tellecaller"
			...
			Enquiry.where(user_id: user.id).where.not(id: @booking).collect { |x| [x.mobile_no + ' - ' + x.name_first+ ' - ' + x.user.employee.first_name, x.id] }
		elsif current_user.role.name == "GroupAdmin"
			@booking = VehicleBooking.all.pluck(:enquiry_id)
			Enquiry.where.not(id: @booking).collect { |x| [x.mobile_no + ' - ' + x.name_first+ ' - ' + x.user.employee.first_name, x.id] }
		else
			...
			Enquiry.where.not(id: @booking).where(user_id: user).collect { |x| [x.mobile_no + ' - ' + x.name_first+ ' - ' + x.user.employee.first_name, x.id] }
		end
	end
end
